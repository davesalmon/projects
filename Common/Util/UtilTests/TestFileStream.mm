...
const char* const kFilePath = "/tmp/testme.txt";
...
@interface TestFileStream : XCTestCase

@end

@implementation TestFileStream
...
- (void)testReadWrite {
	...
	try {
		DlFileStream s(DlFileSpec(kFilePath), DlOpenMode::DlReadWrite);
		...
		XCTAssertEqual(s.GetFile()->FileSize(), count);
	...
	try {
		DlFileStream s(DlFileSpec(kFilePath), DlOpenMode::DlReadOnly);
	...
	try {
		DlFileStream s(DlFileSpec(kFilePath), DlOpenMode::DlWriteOnly);
	} catch(std::exception& ex) {
		XCTFail("Caught exception %s", ex.what());
	}
	
	DlFileSpec::PathDelete(kFilePath, false);
}
